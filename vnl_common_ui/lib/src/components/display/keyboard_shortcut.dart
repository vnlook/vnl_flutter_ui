...
class VNLKeyboardShortcutDisplayHandle {
  final KeyboardShortcutDisplayBuilder _builder;

  /// Creates a handle with the specified builder.
  const VNLKeyboardShortcutDisplayHandle(this._builder);
...
class VNLKeyboardShortcutDisplayMapper extends StatefulWidget {
  ...
  const VNLKeyboardShortcutDisplayMapper({
    super.key,
  ...
  @override
  State<VNLKeyboardShortcutDisplayMapper> createState() =>
      _KeyboardShortcutDisplayMapperState();
}

class _KeyboardShortcutDisplayMapperState
    extends State<VNLKeyboardShortcutDisplayMapper> {
  late VNLKeyboardShortcutDisplayHandle _handle;

  @override
  void initState() {
    super.initState();
    _handle = VNLKeyboardShortcutDisplayHandle(widget.builder);
  }

  @override
  void didUpdateWidget(covariant VNLKeyboardShortcutDisplayMapper oldWidget) {
...
class VNLKeyboardDisplay extends StatelessWidget {
  ...
  /// Creates a [VNLKeyboardDisplay] from a list of keyboard keys.
  ///
  ...
  /// Creates a [VNLKeyboardDisplay] from a shortcut activator.
  ///
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
            ...
            children: keys
                .map((key) => VNLKeyboardKeyDisplay(keyboardKey: key))
                .toList())
        .gap(spacing);
  }
}
...
/// resembles a physical keyboard key. Used internally by [VNLKeyboardDisplay]
...
/// Example:
/// ```dart
/// VNLKeyboardKeyDisplay(
///   keyboardKey: LogicalKeyboardKey.controlLeft,
///   padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
/// )
/// ```
class VNLKeyboardKeyDisplay extends StatelessWidget {
  ...
  /// Creates a [VNLKeyboardKeyDisplay] for the specified keyboard key.
  ///
