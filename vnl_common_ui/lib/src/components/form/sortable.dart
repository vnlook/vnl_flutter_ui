...
/// Parent data for sortable items within a [VNLRawSortableStack].
///
/// Extends [ContainerBoxParentData] to include positioning information
/// for items in a sortable layout.
class VNLRawSortableParentData extends ContainerBoxParentData<RenderBox> {
...
class VNLRawSortableItemPositioned
    extends ParentDataWidget<VNLRawSortableParentData> {
  /// The offset where the item should be positioned.
  final Offset offset;

  /// Creates a [VNLRawSortableItemPositioned].
  ///
  /// Parameters:
  /// - [offset] (`Offset`, required): Position offset for the child.
  /// - [child] (`Widget`, required): The child widget to position.
  const VNLRawSortableItemPositioned({
    super.key,
  ...
  void applyParentData(RenderObject renderObject) {
    final parentData = renderObject.parentData as VNLRawSortableParentData;
  ...
  @override
  Type get debugTypicalAncestorWidgetClass => VNLRawSortableStack;
}

/// VNLRawSortableStack prevents the stacking children from going outside the bounds of this widget.
...
class VNLRawSortableStack extends MultiChildRenderObjectWidget {
  /// Creates a raw sortable stack.
  const VNLRawSortableStack({
    super.key,
  ...
  RenderObject createRenderObject(BuildContext context) {
    return VNLRenderRawSortableStack();
  }

  @override
  void updateRenderObject(
      BuildContext context, VNLRenderRawSortableStack renderObject) {
...
class VNLRenderRawSortableStack extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, VNLRawSortableParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, VNLRawSortableParentData> {
  /// Whether drag-and-drop interactions are enabled.
  bool enabled = true;

  @override
  void setupParentData(RenderBox child) {
    if (child.parentData is! VNLRawSortableParentData) {
      child.parentData = VNLRawSortableParentData();
    }
  }
  ...
  void performLayout() {
    ...
    while (child != null) {
      var childParentData = child.parentData as VNLRawSortableParentData;
