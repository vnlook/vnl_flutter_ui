...
/// Theme configuration for [VNLAutoComplete] widget styling and behavior.
///
...
class VNLAutoCompleteTheme extends ComponentThemeData {
  ...
  bool operator ==(Object other) {
    ...
    return other is VNLAutoCompleteTheme &&
...
class VNLAutoComplete extends StatefulWidget {
  ...
  ///
  /// Typically a [VNLTextField] or similar text input widget. The autocomplete
  ...
  /// Creates an [VNLAutoComplete] widget.
  ///
  ...
  /// [VNLAutoComplete] constructor. Simply returns the input suggestion as-is.
  ...
  void _onSuggestionsChanged() {
    if ((_suggestions.value.isEmpty && _popoverController.hasOpenPopover) ||
      ...
      _popoverController.show(
        context: context,
        handler: const VNLPopoverOverlayHandler(),
        builder: (context) {
            ...
            shortcuts: _popoverController.hasOpenPopover
                ? {
                    LogicalKeySet(LogicalKeyboardKey.arrowDown):
                        const VNLNavigateSuggestionIntent(1),
                    LogicalKeySet(LogicalKeyboardKey.arrowUp):
                        const VNLNavigateSuggestionIntent(-1),
                    if (widget.suggestions.isNotEmpty &&
                        _selectedIndex.value != -1)
                      LogicalKeySet(LogicalKeyboardKey.tab):
                          const VNLAcceptSuggestionIntent(),
                  }
                : null,
            actions: _popoverController.hasOpenPopover
                ? {
                    VNLNavigateSuggestionIntent:
                        CallbackAction<VNLNavigateSuggestionIntent>(
                      onInvoke: (intent) {
                    ...
                    VNLAcceptSuggestionIntent:
                        CallbackAction<VNLAcceptSuggestionIntent>(
                      onInvoke: (intent) {
                        _handleProceed();
                        return;
                      },
...
///
/// Used internally by [VNLAutoComplete] to handle arrow key navigation within
...
class VNLNavigateSuggestionIntent extends Intent {
  ...
  const VNLNavigateSuggestionIntent(this.direction);
}

/// Intent for accepting the currently selected autocomplete suggestion.
///
/// Used internally by [VNLAutoComplete] to handle suggestion acceptance via
...
class VNLAcceptSuggestionIntent extends Intent {
