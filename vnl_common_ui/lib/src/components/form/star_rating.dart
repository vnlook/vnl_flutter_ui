...
/// Theme data for customizing [VNLStarRating] widget appearance.
///
/// This class defines the visual properties that can be applied to
/// [VNLStarRating] widgets, including colors for filled and unfilled stars,
...
class VNLStarRatingTheme extends ComponentThemeData {
  ...
  bool operator ==(Object other) {
    ...
    return other is VNLStarRatingTheme &&
...
/// A controller for managing [VNLStarRating] widget values programmatically.
///
...
/// Example:
/// ```dart
/// final controller = VNLStarRatingController(3.5);
///
/// // Listen to changes
...
/// ```
class VNLStarRatingController extends ValueNotifier<double>
    with ComponentController<double> {
  /// Creates a [VNLStarRatingController] with the given initial [value].
  ///
  ...
  VNLStarRatingController([super.value = 0.0]);
}
...
/// - **VNLStep control**: Configurable rating increments for precision
...
/// final controller = VNLStarRatingController(3.5);
///
/// VNLControlledStarRating(
///   controller: controller,
///   max: 5.0,
///   step: 0.5,
///   activeColor: VNLColors.amber,
/// )
/// ```
///
...
/// )
/// ```
class VNLControlledStarRating extends StatelessWidget
  ...
  final bool enabled;
  @override
  final VNLStarRatingController? controller;
  ...
  /// Creates a [VNLControlledStarRating].
  ///
  ...
  ///
  /// Parameters:
  /// - [controller] (VNLStarRatingController?, optional): external state controller
  ...
  ///   step: 0.1,
  ///   activeColor: VNLColors.amber,
  ///   backgroundColor: VNLColors.grey[300],
  /// )
  /// ```
  const VNLControlledStarRating({
    super.key,
    this.controller,
...
///
/// [VNLStarRating] provides a customizable rating interface using star-shaped
        ...
        return FocusableActionDetector(
          ...
          shortcuts: {
            LogicalKeySet(LogicalKeyboardKey.arrowRight):
                VNLIncreaseStarIntent(widget.step),
            LogicalKeySet(LogicalKeyboardKey.arrowLeft):
                VNLDecreaseStarIntent(widget.step),
          },
          actions: {
            VNLIncreaseStarIntent: CallbackAction<VNLIncreaseStarIntent>(
              onInvoke: (intent) {
            ...
            VNLDecreaseStarIntent: CallbackAction<VNLDecreaseStarIntent>(
              onInvoke: (intent) {
                if (widget.onChanged != null) {
                  widget.onChanged!(
                      (roundedValue - intent.step).clamp(0.0, widget.max));
...
class VNLIncreaseStarIntent extends Intent {
  /// The step size to increase the rating by.
  final double step;

  /// Creates an [VNLIncreaseStarIntent].
  const VNLIncreaseStarIntent(this.step);
}
...
class VNLDecreaseStarIntent extends Intent {
  /// The step size to decrease the rating by.
  final double step;

  /// Creates a [VNLDecreaseStarIntent].
  const VNLDecreaseStarIntent(this.step);
}
