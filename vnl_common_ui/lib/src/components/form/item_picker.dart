...
/// ```dart
/// ItemPicker<Color>(
///   items: ItemList([VNLColors.red, VNLColors.green, VNLColors.blue]),
///   value: VNLColors.red,
...
class ItemPicker<T> extends StatelessWidget {
  ...
  final VNLItemPickerLayout? layout;
...
///
/// Defines an interface for accessing items by index, used by [VNLItemPickerLayout]
...
///
/// See also:
/// - [VNLListItemPickerLayout], which displays items in a vertical list.
/// - [VNLGridItemPickerLayout], which displays items in a grid.
abstract class VNLItemPickerLayout {
  /// A list layout for item pickers.
  static const VNLListItemPickerLayout list = VNLListItemPickerLayout();

  /// A grid layout for item pickers (4 columns by default).
  static const VNLGridItemPickerLayout grid = VNLGridItemPickerLayout();

  /// Creates an [VNLItemPickerLayout].
  const VNLItemPickerLayout();
...
/// Example:
/// ```dart
/// const VNLListItemPickerLayout()
/// ```
class VNLListItemPickerLayout extends VNLItemPickerLayout {
  /// Creates a [VNLListItemPickerLayout].
  const VNLListItemPickerLayout();
  @override
...
/// Example:
/// ```dart
/// VNLGridItemPickerLayout(crossAxisCount: 3)
/// ```
class VNLGridItemPickerLayout extends VNLItemPickerLayout {
  /// Number of columns in the grid.
  final int crossAxisCount;

  /// Creates a [VNLGridItemPickerLayout].
  ///
  /// Parameters:
  /// - [crossAxisCount] (`int`, default: `4`): The number of grid columns.
  const VNLGridItemPickerLayout({this.crossAxisCount = 4});
  ...
  ///
  /// Returns: A new [VNLGridItemPickerLayout] with the specified columns.
  VNLItemPickerLayout call({int crossAxisCount = 4}) {
    return VNLGridItemPickerLayout(crossAxisCount: crossAxisCount);
  }
...
/// - [layout] (`VNLItemPickerLayout`, default: `VNLGridItemPickerLayout()`): Layout strategy.
/// - [alignment] (`AlignmentGeometry?`, optional): VNLPopover alignment.
...
Future<T?> showItemPicker<T>(
  ...
  T? initialValue,
  VNLItemPickerLayout layout = const VNLGridItemPickerLayout(),
...
///   items: ItemList([VNLColors.red, VNLColors.green]),
///   builder: (context, color) => ColorTile(color),
///   layout: VNLItemPickerLayout.grid,
...
class ItemPickerDialog<T> extends StatefulWidget {
  ...
  const ItemPickerDialog({
    ...
    this.value,
    this.onChanged,
    this.layout = const VNLGridItemPickerLayout(),
  });
...
class VNLItemPickerData {
  ...
  /// Creates an [VNLItemPickerData].
  ///
  /// Parameters:
  /// - [value] (`Object?`, optional): Current selection.
  /// - [onChanged] (`ValueChanged<Object?>?`, optional): Change callback.
  /// - [layout] (`VNLItemPickerLayout`, required): Layout strategy.
  const VNLItemPickerData({this.value, this.onChanged, required this.layout});

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    if (other is! VNLItemPickerData) return false;
...
///   value: VNLColors.red,
///   child: Container(color: VNLColors.red, width: 50, height: 50),
/// )
/// ```
class ItemPickerOption<T> extends StatelessWidget {
  ...
  /// - [style] (`VNLAbstractButtonStyle?`, optional): Style when not selected.
  /// - [selectedStyle] (`VNLAbstractButtonStyle?`, optional): Style when selected.
  ...
  @override
  Widget build(BuildContext context) {
    final data = Data.maybeOf<VNLItemPickerData>(context);
    if (data == null) {
    ...
    if (data.layout is VNLListItemPickerLayout) {
      if (label == null) {
        return VNLButton(
          onPressed:
              data.onChanged == null ? null : () => data.onChanged!(value),
          style: data.value == value
              ? (selectedStyle ?? VNLButtonVariance.primary)
              : (style ?? VNLButtonVariance.ghost),
          child: child,
        );
      }
      return VNLButton(
