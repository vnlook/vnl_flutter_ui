...
abstract class Validator<T> {
  ...
  /// null if the value is valid, or a [VNLValidationResult] describing the
  ...
  /// Example:
  /// ```dart
  /// final validator = RequiredValidator<String>() & VNLEmailValidator();
  /// ```
    ...
    return MultiData(
      data: ignoring
          ? const [
              ...
              Data<VNLFormController>.boundary(),
            ]
          : const [],
      child: child,
    );
  ...
  FutureOr<VNLValidationResult?> validate(
    ...
    if (result is Future<bool>) {
      return result.then((value) {
        if (!value) {
          return VNLInvalidResult(message, state: lifecycle);
        }
        return null;
      });
    } else if (!result) {
...
///
/// Returns a `FutureOr<VNLValidationResult?>` that is null for valid values.
typedef ValidatorBuilderFunction<T> = FutureOr<VNLValidationResult?> Function(
    T? value);
...
///       return null; // Valid
///     }
///     return VNLInvalidResult('Must contain @');
///   },
/// )
/// ```
...
class NotValidator<T> extends Validator<T> {
  ...
  FutureOr<VNLValidationResult?> validate(
    ...
    if (result is Future<VNLValidationResult?>) {
      return result.then((value) {
        if (value == null) {
          return VNLInvalidResult(message ?? localizations.invalidValue,
              state: state);
        }
        return null;
      });
    } else if (result == null) {
      return VNLInvalidResult(message ?? localizations.invalidValue, state: state);
    }
    return null;
  }
...
class VNLNotEmptyValidator extends NonNullValidator<String> {
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLNotEmptyValidator && other.message == message;
  }
...
///
/// [VNLLengthValidator] validates that a string's length falls within the minimum
...
/// )
/// ```
class VNLLengthValidator extends Validator<String> {
  ...
  /// Creates a [VNLLengthValidator] with optional min/max bounds.
  const VNLLengthValidator({this.min, this.max, this.message});
  ...
  FutureOr<VNLValidationResult?> validate(
    ...
    if (min != null && value.length < min!) {
      return VNLInvalidResult(message ?? localizations.formLengthLessThan(min!),
          state: state);
    }
    if (max != null && value.length > max!) {
      return VNLInvalidResult(message ?? localizations.formLengthGreaterThan(max!),
          state: state);
    }
    return null;
  }
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLLengthValidator &&
        other.min == min &&
        other.max == max &&
        other.message == message;
  }
...
///
/// [VNLSafePasswordValidator] checks passwords against common security criteria:
...
/// )
/// ```
class VNLSafePasswordValidator extends Validator<String> {
  ...
  /// Creates a [VNLSafePasswordValidator] with configurable requirements.
  const VNLSafePasswordValidator(
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLSafePasswordValidator &&
...
///
/// [VNLRegexValidator] provides flexible pattern-based validation using regular
...
/// )
/// ```
class VNLRegexValidator extends Validator<String> {
  ...
  /// Creates a [VNLRegexValidator] with the specified pattern.
  const VNLRegexValidator(this.pattern, {this.message});
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLRegexValidator &&
...
///
/// [VNLEmailValidator] uses the email_validator package to validate email
...
/// )
/// ```
class VNLEmailValidator extends Validator<String> {
  /// Custom error message, or null to use default localized message.
  final String?
      message; // if null, use default message from VNLookLocalizations

  /// Creates an [VNLEmailValidator] with an optional custom message.
  const VNLEmailValidator({this.message});
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLEmailValidator && other.message == message;
  }
...
///
/// [VNLURLValidator] validates URLs using Dart's Uri parsing capabilities
...
/// )
/// ```
class VNLURLValidator extends Validator<String> {
  /// Custom error message, or null to use default localized message.
  final String?
      message; // if null, use default message from VNLookLocalizations

  /// Creates a [VNLURLValidator] with an optional custom message.
  const VNLURLValidator({this.message});
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLURLValidator && other.message == message;
  }
...
///
/// [VNLValidationResult] encapsulates the outcome of validating a form field value.
/// Subclasses include [VNLInvalidResult] for validation failures and [ValidResult]
...
abstract class VNLValidationResult {
  ...
  VNLValidationResult attach(FormKey key);
}
...
class ReplaceResult<T> extends VNLValidationResult {
...
///
/// [VNLInvalidResult] contains an error message describing why validation failed.
/// This is the most common validation result type returned by validators when
/// a value doesn't meet the validation criteria.
class VNLInvalidResult extends VNLValidationResult {
  /// The error message describing the validation failure.
  final String message;

  final FormKey? _key;

  /// Creates an [VNLInvalidResult] with the specified error message.
  const VNLInvalidResult(this.message, {required super.state}) : _key = null;

  /// Creates an [VNLInvalidResult] already attached to a form field key.
  const VNLInvalidResult.attached(this.message,
      {required FormKey key, required super.state})
      : _key = key;
  ...
  @override
  VNLInvalidResult attach(FormKey key) {
    return VNLInvalidResult.attached(message, key: key, state: state);
  }
}

/// A notification sent when a form field's validation state changes.
///
/// [VNLFormValidityNotification] is dispatched through the notification system
...
class VNLFormValidityNotification extends Notification {
  /// The previous validation result, or null if there was none.
  final VNLValidationResult? oldValidity;

  /// The new validation result, or null if now valid.
  final VNLValidationResult? newValidity;

  /// Creates a [VNLFormValidityNotification] with old and new validity states.
  const VNLFormValidityNotification(this.newValidity, this.oldValidity);
}
...
/// VNLForm key type for color picker fields with [Color] values.
typedef ColorPickerKey = FormKey<Color>;

/// VNLForm key type for date picker fields with [DateTime] values.
typedef DatePickerKey = FormKey<DateTime>;

/// VNLForm key type for date input fields with [DateTime] values.
typedef DateInputKey = FormKey<DateTime>;

/// VNLForm key type for duration picker fields with [Duration] values.
typedef DurationPickerKey = FormKey<Duration>;

/// VNLForm key type for duration input fields with [Duration] values.
typedef DurationInputKey = FormKey<Duration>;

/// VNLForm key type for text input fields with string values.
typedef InputKey = FormKey<String>;

/// VNLForm key type for OTP input fields with lists of nullable integers.
typedef InputOTPKey = FormKey<List<int?>>;

/// VNLForm key type for multi-select fields with iterable values.
typedef MultiSelectKey<T> = FormKey<Iterable<T>>;

/// VNLForm key type for multiple answer fields with iterable values.
typedef MultipleAnswerKey<T> = FormKey<Iterable<T>>;

/// VNLForm key type for multiple choice fields with single selected values.
typedef MultipleChoiceKey<T> = FormKey<T>;

/// VNLForm key type for number input fields with numeric values.
typedef NumberInputKey = FormKey<num>;

/// VNLForm key type for phone input fields with [VNLPhoneNumber] values.
typedef PhoneInputKey = FormKey<VNLPhoneNumber>;

/// VNLForm key type for radio card fields with integer index values.
typedef RadioCardKey = FormKey<int>;

/// VNLForm key type for radio group fields with integer index values.
typedef RadioGroupKey = FormKey<int>;

/// VNLForm key type for select dropdown fields with typed values.
typedef SelectKey<T> = FormKey<T>;

/// VNLForm key type for slider fields with [VNLSliderValue] values.
typedef SliderKey = FormKey<VNLSliderValue>;

/// VNLForm key type for star rating fields with double values.
typedef StarRatingKey = FormKey<double>;

/// VNLForm key type for switch fields with boolean values.
typedef SwitchKey = FormKey<bool>;

/// VNLForm key type for text area fields with string values.
typedef TextAreaKey = FormKey<String>;

/// VNLForm key type for text field inputs with string values.
typedef TextFieldKey = FormKey<String>;

/// VNLForm key type for time picker fields with [TimeOfDay] values.
typedef TimePickerKey = FormKey<TimeOfDay>;

/// VNLForm key type for time input fields with [TimeOfDay] values.
typedef TimeInputKey = FormKey<TimeOfDay>;

/// VNLForm key type for toggle fields with boolean values.
...
class FormEntry<T> extends StatefulWidget {
  ...
  @override
  State<FormEntry> createState() => VNLFormEntryState();
}
...
mixin FormFieldHandle {
  ...
  ///
  /// Returns: `FutureOr<VNLValidationResult?>` — validation result if applicable.
  FutureOr<VNLValidationResult?> reportNewFormValue<T>(T? value);

  /// Re-runs validation on the current value.
  ///
  /// Returns: `FutureOr<VNLValidationResult?>` — validation result if applicable.
  FutureOr<VNLValidationResult?> revalidate();

  /// A listenable for the current validation state.
  ///
  /// Returns `null` if no validation has been performed or if validation passed.
  ValueListenable<VNLValidationResult?>? get validity;
}
...
///
/// Manages form field lifecycle and integrates with parent [VNLFormController]
/// for validation and value reporting.
class VNLFormEntryState extends State<FormEntry> with FormFieldHandle {
  VNLFormController? _controller;
  _FormEntryCachedValue? _cachedValue;
  final ValueNotifier<VNLValidationResult?> _validity = ValueNotifier(null);

  @override
  FormKey get formKey => widget.key;

  @override
  ValueListenable<VNLValidationResult?>? get validity => _validity;

  int _toWaitCounter = 0;
  FutureOr<VNLValidationResult?>? _toWait;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    var oldController = _controller;
    var newController = Data.maybeOf<VNLFormController>(context);
  ...
  void _onControllerChanged() {
    ...
    if (_toWait is Future<VNLValidationResult?>) {
      (_toWait as Future<VNLValidationResult?>).then((value) {
        if (counter == _toWaitCounter) {
          _validity.value = value;
        }
      });
    } else {
      _validity.value = _toWait as VNLValidationResult?;
    }
  }
...
class _FormEntryHandleInterceptor with FormFieldHandle {
  ...
  @override
  ValueListenable<VNLValidationResult?>? get validity => handle?.validity;
...
///
/// The VNLForm widget creates a container that manages multiple form fields, providing
/// centralized validation, data collection, and submission handling. It maintains
/// form state through a [VNLFormController] and coordinates validation across all
/// participating form fields.
///
/// VNLForm components within the widget tree automatically register themselves with
/// the nearest VNLForm ancestor, allowing centralized management of field values,
/// validation states, and error handling. The VNLForm provides validation lifecycle
/// management and supports both synchronous and asynchronous validation.
///
/// Example:
/// ```dart
/// final controller = VNLFormController();
///
/// VNLForm(
///   controller: controller,
///   onSubmit: (values) async {
///     print('VNLForm submitted with values: $values');
///   },
///   child: Column(
///     children: [
///       TextInput(
...
class VNLForm extends StatefulWidget {
  /// Retrieves the nearest [VNLFormController] from the widget tree, if any.
  ///
  /// Returns the [VNLFormController] instance from the nearest VNLForm ancestor,
  /// or null if no VNLForm is found in the widget tree. This method is safe
  /// to call even when no VNLForm is present.
  ///
  /// Parameters:
  /// - [context] (BuildContext): The build context to search from
  ///
  /// Returns the [VNLFormController] if found, null otherwise.
  static VNLFormController? maybeOf(BuildContext context) {
    return Data.maybeOf<VNLFormController>(context);
  }

  /// Retrieves the nearest [VNLFormController] from the widget tree.
  ///
  /// Returns the [VNLFormController] instance from the nearest VNLForm ancestor.
  /// Throws an assertion error if no VNLForm is found in the widget tree.
  /// Use [maybeOf] if the VNLForm might not be present.
  ///
  /// Parameters:
  /// - [context] (BuildContext): The build context to search from
  ///
  /// Returns the [VNLFormController] from the nearest VNLForm ancestor.
  ///
  /// Throws [AssertionError] if no VNLForm is found in the widget tree.
  static VNLFormController of(BuildContext context) {
    return Data.of<VNLFormController>(context);
  }
  ...
  /// If null, the VNLForm creates and manages its own internal controller.
  final VNLFormController? controller;

  /// The widget subtree containing form fields.
  ///
  /// This child widget should contain the form fields and other UI elements
  /// that participate in the form. VNLForm fields within this subtree automatically
  /// register with this VNLForm instance.
  final Widget child;

  /// Callback invoked when the form is submitted.
  ///
  /// This callback receives a map of form values keyed by their [FormKey]
  /// identifiers. It is called when [VNLFormController.submit] is invoked and
  ...
  /// - [controller] (VNLFormController?, optional): External form state controller
  ///
  /// Example:
  /// ```dart
  /// VNLForm(
  ...
  @override
  State<VNLForm> createState() => VNLFormState();
}
...
///
/// The VNLFormController coordinates all form field interactions, maintaining
...
///   print('VNLForm validity: ${controller.isValid}');
///   print('VNLForm values: ${controller.values}');
/// });
///
/// // Submit the form
...
/// ```
class VNLFormController extends ChangeNotifier {
  ...
  /// form fields. Values can be either synchronous VNLValidationResult objects
  /// or `Future<VNLValidationResult?>` for asynchronous validation.
  ///
  /// Returns a `Map<FormKey, FutureOr<VNLValidationResult?>>` representing the
  /// current validation state of all form fields.
  Map<FormKey, FutureOr<VNLValidationResult?>> get validities {
  ...
  /// a [VNLWaitingResult] is included to indicate the validation is in progress.
  ///
  /// Returns a `Map<FormKey, VNLValidationResult>` containing only fields with errors.
  Map<FormKey, VNLValidationResult> get errors {
    final errors = <FormKey, VNLValidationResult>{};
    for (var entry in _validity.entries) {
      var result = entry.value.result;
      if (result is Future<VNLValidationResult?>) {
        errors[entry.key] =
            VNLWaitingResult.attached(state: entry.value.state, key: entry.key);
      } else if (result != null) {
  ...
  /// which can be either a synchronous VNLValidationResult or a Future for asynchronous
  ...
  FutureOr<VNLValidationResult?>? getError(FormKey key) {
  ...
  ///
  /// This method returns the current validation state for the specified form key,
  /// converting asynchronous validations to [VNLWaitingResult] objects. This provides
  ...
  VNLValidationResult? getSyncError(FormKey key) {
    var entry = _validity[key];
    var result = entry?.result;
    if (result is Future<VNLValidationResult?>) {
      return VNLWaitingResult.attached(state: entry!.state, key: key);
    }
    return result;
  }
    ...
    for (var entry in _attachedInputs.entries) {
      ...
      if (value.validator != null) {
        ...
        if (_validity[key]?.result != future) {
          if (future is Future<VNLValidationResult?>) {
  ...
  ///
  /// Returns: `FutureOr<VNLValidationResult?>` — validation result if applicable.
  FutureOr<VNLValidationResult?> attach(BuildContext context,
    ...
    Map<FormKey, FutureOr<VNLValidationResult?>> revalidate = {};
    for (var entry in _attachedInputs.entries) {
      var k = entry.key;
      var value = entry.value;
      if (key == k) {
...
/// State class for the [VNLForm] widget that manages form controller lifecycle.
///
/// This state class is responsible for initializing and updating the
/// [VNLFormController] used by the [VNLForm] widget. It ensures proper controller
...
/// - Creates a default [VNLFormController] if none is provided
/// - Updates to a new controller when the widget's controller property changes
///
/// See also:
/// - [VNLForm], the widget that uses this state
/// - [VNLFormController], the controller managed by this state
class VNLFormState extends State<VNLForm> {
  late VNLFormController _controller;

  @override
  void initState() {
    super.initState();
    _controller = widget.controller ?? VNLFormController();
  }
...
class VNLFormEntryErrorBuilder extends StatelessWidget {
  /// Builder function that creates the error display widget.
  final Widget Function(
      BuildContext context, VNLValidationResult? error, Widget? child) builder;
...
class VNLWaitingResult extends VNLValidationResult {
  final FormKey? _key;

  /// Creates a waiting result attached to a form key.
  const VNLWaitingResult.attached({required FormKey key, required super.state})
      : _key = key;
  ...
  @override
  VNLWaitingResult attach(FormKey key) {
    return VNLWaitingResult.attached(key: key, state: state);
  }
}
...
class VNLFormErrorBuilder extends StatelessWidget {
  /// Optional child widget passed to the builder.
  final Widget? child;

  /// Builder function that creates the error display from all form errors.
  final Widget Function(BuildContext context,
      Map<FormKey, VNLValidationResult> errors, Widget? child) builder;

  /// Creates a form error builder.
  const VNLFormErrorBuilder({super.key, required this.builder, this.child});

  @override
  Widget build(BuildContext context) {
    final formController = Data.of<VNLFormController>(context);
    return ListenableBuilder(
...
class VNLFormPendingBuilder extends StatelessWidget {
  ...
  const VNLFormPendingBuilder({super.key, required this.builder, this.child});

  @override
  Widget build(widgets.BuildContext context) {
    final controller = Data.maybeOf<VNLFormController>(context);
    if (controller != null) {
      return AnimatedBuilder(
        ...
        builder: (context, child) {
          ...
          final pending = <FormKey, Future<VNLValidationResult?>>{};
          for (var entry in errors.entries) {
            var key = entry.key;
            var value = entry.value;
            if (value is Future<VNLValidationResult?>) {
              pending[key] = value;
            }
          }
...
extension FormExtension on BuildContext {
  ...
  T? getFormValue<T>(FormKey<T> key) {
    final formController = Data.maybeFind<VNLFormController>(this);
    if (formController != null) {
  ...
  ///
  /// Returns a [VNLSubmissionResult] with form values and any validation errors.
  /// May return a Future if asynchronous validation is in progress.
  FutureOr<VNLSubmissionResult> submitForm() {
    final formState = Data.maybeFind<VNLFormState>(this);
    assert(formState != null, 'VNLForm not found');
    final formController = Data.maybeFind<VNLFormController>(this);
    assert(formController != null, 'VNLForm not found');
    ...
    if (result is Future<VNLSubmissionResult>) {
      return result.then((value) {
  ...
  FutureOr<VNLSubmissionResult> _chainedSubmitForm(
      Map<FormKey, Object?> values,
      Map<FormKey, VNLValidationResult> errors,
      Iterator<MapEntry<FormKey, _ValidatorResultStash>> iterator) {
    if (!iterator.moveNext()) {
      return VNLSubmissionResult(values, errors);
    }
...
mixin FormValueSupplier<T, X extends StatefulWidget> on State<X> {
  ...
  void _reportNewFormValue(T? value) {
    ...
    if (validationResult is Future<VNLValidationResult?>) {
...
class VNLSubmissionResult {
  ...
  final Map<FormKey, VNLValidationResult> errors;

  /// Creates a submission result.
  const VNLSubmissionResult(this.values, this.errors);
  ...
  @override
  bool operator ==(Object other) {
    return other is VNLSubmissionResult &&
...
class VNLFormTableLayout extends StatelessWidget {
  ...
  /// Creates a [VNLFormTableLayout].
  ///
  /// Parameters:
  /// - [rows] (`List<FormField>`, required): VNLForm fields to arrange in rows.
  /// - [spacing] (`double?`, optional): Custom row spacing.
  const VNLFormTableLayout({super.key, required this.rows, this.spacing});
...
/// )
/// ```
class VNLSubmitButton extends StatelessWidget {
  /// VNLButton style configuration.
  ...
  /// Creates a [VNLSubmitButton].
  ///
  /// Parameters:
  /// - [child] (`Widget`, required): Default button content.
  /// - [style] (`VNLAbstractButtonStyle?`, optional): VNLButton styling.
    ...
    return VNLFormErrorBuilder(
      builder: (context, errors, child) {
        var hasWaitingError = errors.values.any((element) {
          return element is VNLWaitingResult;
        });
        var hasError = errors.values.any((element) {
          return element is VNLInvalidResult;
        });
        if (hasWaitingError) {
          return VNLButton(
            ...
            focusNode: focusNode,
            style: style ?? const VNLButtonStyle.primary(),
            child: loading ?? child!,
          );
        }
        if (hasError) {
