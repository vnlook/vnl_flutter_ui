...
/// )
/// ```
abstract class VNLStatedWidget extends StatelessWidget {
  ...
  const VNLStatedWidget._({super.key});

  /// Creates a [VNLStatedWidget] with explicit state-specific widgets.
  ///
  ...
  ///   selected: Icon(Icons.star, color: VNLColors.yellow),
  ///   hovered: Icon(Icons.star_border, color: VNLColors.grey),
  ///   disabled: Icon(Icons.star_border, color: VNLColors.grey.shade300),
  /// )
  /// ```
  const factory VNLStatedWidget({
    Key? key,
  ...
  /// Creates a [VNLStatedWidget] using a map-based state configuration.
  ///
  ...
  /// VNLStatedWidget.map(
  ///   states: {
  ///     WidgetState.selected: Icon(Icons.check_circle, color: VNLColors.green),
  ///     WidgetState.error: Icon(Icons.error, color: VNLColors.red),
  ///     'custom': Icon(Icons.star, color: VNLColors.blue),
  ///   },
  ///   child: Icon(Icons.circle_outlined),
  /// )
  /// ```
  const factory VNLStatedWidget.map({
    Key? key,
    required Map<Object, Widget> states,
    Widget? child,
  }) = _MapStatedWidget;

  /// Creates a [VNLStatedWidget] using a builder function for dynamic state handling.
  ///
  ...
  /// Example:
  /// ```dart
  /// VNLStatedWidget.builder(
  ///   builder: (context, states) {
  ///     if (states.contains(WidgetState.disabled)) {
  ///       return Opacity(opacity: 0.5, child: Icon(Icons.block));
  ///     }
  ///     if (states.contains(WidgetState.selected)) {
  ///       return Icon(Icons.check_circle, color: VNLColors.green);
  ///     }
  ...
  /// )
  /// ```
  const factory VNLStatedWidget.builder({
    Key? key,
...
class _ParamStatedWidget extends VNLStatedWidget {
  ...
  const _ParamStatedWidget({
    super.key,
    this.order = VNLStatedWidget.defaultStateOrder,
    this.child,
    this.disabled,
    this.selected,
    this.pressed,
  ...
  Widget build(BuildContext context) {
    VNLWidgetStatesData? statesData = Data.maybeOf<VNLWidgetStatesData>(context);
...
///
/// [VNLWidgetStatesProvider] manages and propagates widget states (like hovered,
...
/// ## Overview
///
/// Use [VNLWidgetStatesProvider] to:
...
///
/// ```dart
/// VNLWidgetStatesProvider(
///   states: {WidgetState.hovered},
///   child: VNLStatedWidget(
///     child: Text('Normal'),
///     hovered: Text('Hovered'),
///   ),
/// )
/// ```
class VNLWidgetStatesProvider extends StatelessWidget {
  ...
  const VNLWidgetStatesProvider.boundary({
    super.key,
    required this.child,
  })  : boundary = true,
        controller = null,
  ...
  Widget build(BuildContext context) {
    if (boundary) {
      return Data<VNLWidgetStatesData>.boundary(
        child: child,
      );
    }
    Set<WidgetState>? parentStates;
    if (inherit) {
      VNLWidgetStatesData? parentData = Data.maybeOf<VNLWidgetStatesData>(context);
    ...
    return ListenableBuilder(
      ...
      builder: (context, child) {
        ...
        return Data<VNLWidgetStatesData>.inherit(
          data: VNLWidgetStatesData(currentStates),
          child: child!,
        );
      },
      child: child,
...
///
/// [VNLWidgetStatesData] is a simple container for a `Set<WidgetState>` that can
/// be passed through the widget tree using the [Data] inherited widget system.
/// It's used by components like [VNLClickable] to propagate state information
/// (hovered, pressed, focused, etc.) to descendant widgets.
///
/// Example:
/// ```dart
/// const statesData = VNLWidgetStatesData({WidgetState.hovered, WidgetState.focused});
/// ```
class VNLWidgetStatesData {
  ...
  const VNLWidgetStatesData(this.states);

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is VNLWidgetStatesData && setEquals(states, other.states);
  }
  ...
  @override
  String toString() => 'VNLWidgetStatesData(states: $states)';
}

class _MapStatedWidget extends VNLStatedWidget {
...
class _BuilderStatedWidget extends VNLStatedWidget {
  ...
  @override
  Widget build(BuildContext context) {
    VNLWidgetStatesData? statesData = Data.maybeOf(context);
    Set<WidgetState> states = statesData?.states ?? {};
...
///
/// [VNLClickable] provides a comprehensive foundation for interactive widgets, handling
...
/// ## Overview
///
/// Use [VNLClickable] when building custom interactive components that need:
...
///       return BoxDecoration(color: VNLColors.blue.shade700);
///     }
///     return BoxDecoration(color: VNLColors.blue);
///   }),
///   child: Padding(
...
/// )
/// ```
class VNLClickable extends StatefulWidget {
  ...
  @override
  State<VNLClickable> createState() => _ClickableState();
}
...
/// as part of a multi-tap gesture sequence. Used internally by [VNLClickable] to
/// detect double-tap gestures.
const kDoubleTapMinTime = Duration(milliseconds: 300);

class _ClickableState extends State<VNLClickable> {
  ...
  @override
  void didUpdateWidget(covariant VNLClickable oldWidget) {
  ...
  Widget _builder(BuildContext context, Widget? _) {
    ...
    var widgetStates = Data.maybeOf<VNLWidgetStatesData>(context)?.states ?? {};
