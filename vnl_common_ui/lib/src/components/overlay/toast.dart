...
class ToastTheme extends ComponentThemeData {
  ...
  ///
  /// Takes a [BuildContext] and [VNLToastOverlay] instance, returning the widget
  ...
  /// Example:
  /// ```dart
  /// ToastBuilder builder = (context, overlay) => VNLCard(
  ///   child: ListTile(
  ///     title: Text('Notification'),
  ///     trailing: VNLIconButton(
...
typedef ToastBuilder = Widget Function(
    BuildContext context, VNLToastOverlay overlay);
...
/// within the nearest [VNLToastLayer] in the widget tree. The toast appears at
...
///
/// Returns:
/// A [VNLToastOverlay] instance that provides control methods for the displayed toast.
///
/// Throws:
/// - [AssertionError] if no [VNLToastLayer] is found in the widget tree.
///
/// Example:
/// ```dart
...
VNLToastOverlay showToast({
  ...
  assert(layer != null, 'No VNLToastLayer found in context');
  final entry = VNLToastEntry(
    builder: builder,
    location: location,
...
///
/// VNLToastLayer serves as the foundation for the toast notification system,
...
/// );
/// ```
class VNLToastLayer extends StatefulWidget {
  ...
  @override
  State<VNLToastLayer> createState() => _ToastLayerState();
}
...
class _ToastLayerState extends State<VNLToastLayer> {
  ...
  VNLToastOverlay addEntry(VNLToastEntry entry) {
  ...
  void removeEntry(VNLToastEntry entry) {
...
///
/// VNLToastOverlay provides methods for managing the lifecycle and state of
...
abstract class VNLToastOverlay {
...
class _AttachedToastEntry implements VNLToastOverlay {
  final GlobalKey<_ToastEntryLayoutState> key = GlobalKey();
  final VNLToastEntry entry;
...
class VNLToastEntryLayout extends StatefulWidget {
  ...
  @override
  State<VNLToastEntryLayout> createState() => _ToastEntryLayoutState();
}

class _ToastEntryLayoutState extends State<VNLToastEntryLayout> {
