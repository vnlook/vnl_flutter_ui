...
/// ComponentThemeData(
///   data: {
///     HoverCardTheme: VNLHoverCardTheme(
...
/// )
/// ```
class VNLHoverCardTheme extends ComponentThemeData {
  ...
  ///
  /// Returns: `VNLHoverCardTheme` — new theme with updated values.
  VNLHoverCardTheme copyWith({
  ...
  bool operator ==(Object other) {
    return other is VNLHoverCardTheme &&
...
///
/// Features:
/// - VNLHover-based popover display with timing controls
...
class VNLHoverCard extends StatefulWidget {
  ...
  final VNLPopoverController? controller;

  /// Custom overlay handler for popover display.
  final VNLOverlayHandler? handler;

  /// Creates a [VNLHoverCard].
  ///
  ...
  /// - [controller] (VNLPopoverController?, optional): controller for programmatic control
  ...
  /// VNLHoverCard(
  ///   debounce: Duration(milliseconds: 300),
  ///   hoverBuilder: (context) => VNLTooltip(message: 'Help text'),
...
class _HoverCardState extends State<VNLHoverCard> {
  late VNLPopoverController _controller;
  ...
  void initState() {
    super.initState();
    _controller = widget.controller ?? VNLPopoverController();
  }
  ...
  void _showPopover(BuildContext context,
    ...
    VNLOverlayHandler? handler = widget.handler;
    if (handler == null) {
      final overlayManager = VNLOverlayManager.of(context);
      handler =
          VNLOverlayManagerAsTooltipOverlayHandler(overlayManager: overlayManager);
    }
    _controller.show(
      context: context,
      builder: (context) {
