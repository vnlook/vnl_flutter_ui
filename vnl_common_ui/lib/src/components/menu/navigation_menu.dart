...
/// Theme configuration for [VNLNavigationMenu] components.
///
...
class VNLNavigationMenuTheme extends ComponentThemeData {
  ...
  bool operator ==(Object other) {
    ...
    return other is VNLNavigationMenuTheme &&
...
/// An individual menu item within a [VNLNavigationMenu].
///
...
///
/// The item automatically integrates with its parent [VNLNavigationMenu] to
...
///   content: VNLNavigationMenuContent(
///     title: Text('Products'),
...
/// )
/// ```
class VNLNavigationMenuItem extends StatefulWidget {
  ...
  /// Creates a [VNLNavigationMenuItem] with the specified properties.
  ///
  ...
  @override
  State<VNLNavigationMenuItem> createState() => VNLNavigationMenuItemState();
}

/// State class for [VNLNavigationMenuItem] widget.
///
/// Manages the lifecycle and interaction of a single navigation menu item.
class VNLNavigationMenuItemState extends State<VNLNavigationMenuItem> {
  VNLNavigationMenuState? _menuState;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    var newMenuState = Data.maybeOf<VNLNavigationMenuState>(context);
    assert(newMenuState != null,
        'VNLNavigationItem must be a descendant of VNLNavigationMenu');
  ...
  @override
  void didUpdateWidget(covariant VNLNavigationMenuItem oldWidget) {
...
/// This widget is designed to be used within [VNLNavigationMenuItem] content
...
/// )
/// ```
class VNLNavigationMenuContent extends StatelessWidget {
  ...
  /// Creates a [VNLNavigationMenuContent] with the specified properties.
  ///
  ...
  /// )
  /// ```
  const VNLNavigationMenuContent({
    super.key,
    required this.title,
    this.content,
    this.leading,
...
///
/// Provides flexible grid-based layout for multiple [VNLNavigationMenuContent]
...
/// Example:
/// ```dart
/// VNLNavigationMenuContentList(
///   crossAxisCount: 2,
///   spacing: 16.0,
///   runSpacing: 12.0,
///   children: [
///     VNLNavigationMenuContent(title: Text('Dashboard'), onPressed: _openDashboard),
///     VNLNavigationMenuContent(title: Text('Analytics'), onPressed: _openAnalytics),
///     VNLNavigationMenuContent(title: Text('Settings'), onPressed: _openSettings),
///   ],
/// )
/// ```
class VNLNavigationMenuContentList extends StatelessWidget {
  /// The list of widgets to arrange in the grid layout.
  ///
  /// Typically contains [VNLNavigationMenuContent] items or other
  ...
  /// Creates a [VNLNavigationMenuContentList] with the specified layout properties.
  ///
  ...
  /// )
  /// ```
  const VNLNavigationMenuContentList({
    super.key,
...
///         children: [
///           VNLNavigationMenuContent(title: Text('Web Apps')),
///           VNLNavigationMenuContent(title: Text('Mobile Apps')),
///         ],
///       ),
///     ),
///     VNLNavigationMenuItem(
...
class VNLNavigationMenu extends StatefulWidget {
  ...
  ///
  /// Each item should be a [VNLNavigationMenuItem] that defines the
  /// menu's structure and behavior. Items can have content for
  /// dropdown functionality or simple press actions.
  final List<Widget> children;

  /// Creates a [VNLNavigationMenu] with the specified items and appearance.
  ///
  /// The [children] parameter is required and should contain
  /// [VNLNavigationMenuItem] widgets that define the menu structure.
  /// Appearance properties are optional and will use theme defaults.
  ///
  /// Parameters:
  /// - [surfaceOpacity] (double?, optional): VNLPopover background opacity
  /// - [surfaceBlur] (double?, optional): VNLPopover backdrop blur intensity
  ...
  ///   surfaceOpacity: 0.9,
  ///   children: [
  ///     VNLNavigationMenuItem(child: Text('Home'), onPressed: _goHome),
  ///     VNLNavigationMenuItem(child: Text('About'), onPressed: _showAbout),
  ///   ],
  /// )
  /// ```
  ...
  @override
  State<VNLNavigationMenu> createState() => VNLNavigationMenuState();
}

/// State class for [VNLNavigationMenu] managing menu interactions and timing.
///
...
class VNLNavigationMenuState extends State<VNLNavigationMenu> {
  ...
  final Map<VNLNavigationMenuItemState, WidgetBuilder> _contentBuilders = {};

  int _hoverCount = 0;

  void _attachContentBuilder(
      VNLNavigationMenuItemState key, WidgetBuilder builder) {
  ...
  ///
  /// Parameters:
  /// - [item] (`VNLNavigationMenuItemState`, required): the menu item to check
  ///
  /// Returns: `bool` — true if the item is active and popover is open
  bool isActive(VNLNavigationMenuItemState item) {
  ...
  void _activate(VNLNavigationMenuItemState item) {
  ...
  ///
  /// Returns: `VNLNavigationMenuItemState?` — the state if found, null otherwise
  VNLNavigationMenuItemState? findByWidget(Widget widget) {
  ...
  Widget buildContent(int index) {
    VNLNavigationMenuItemState? item = findByWidget(widget.children[index]);
    final theme = Theme.of(context);
    final scaling = theme.scaling;
    final densityContentPadding = theme.density.baseContentPadding * scaling;
    if (item != null) {
      return Data<VNLNavigationMenuState>.boundary(
        child: Padding(
