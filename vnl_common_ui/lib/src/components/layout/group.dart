...
/// Children are positioned using [VNLGroupPositioned] widgets that specify their
...
/// )
/// ```
class VNLGroupWidget extends MultiChildRenderObjectWidget {
  /// Creates a [VNLGroupWidget].
  const VNLGroupWidget({
    super.key,
    super.children,
  });
  ...
  RenderObject createRenderObject(BuildContext context) {
    return VNLRenderGroup();
  }

  @override
  void updateRenderObject(BuildContext context, VNLRenderGroup renderObject) {}
}

/// Parent data for children of [VNLGroupWidget].
///
/// Stores positioning and sizing information for each child widget within
/// a [VNLGroupWidget]. These values are set by [VNLGroupPositioned].
class VNLGroupParentData extends ContainerBoxParentData<RenderBox> {
...
/// Render object for [VNLGroupWidget] that handles absolute positioning of children.
///
/// Manages layout and painting of children positioned using [VNLGroupParentData].
class VNLRenderGroup extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, VNLGroupParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, VNLGroupParentData> {
  /// Creates a [VNLRenderGroup].
  ///
  /// Parameters:
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! VNLGroupParentData) {
      child.parentData = VNLGroupParentData();
    }
  }
  ...
  void performLayout() {
    ...
    while (child != null) {
      final childParentData = child.parentData as VNLGroupParentData;
...
/// Positions a child widget within a [VNLGroupWidget].
///
...
/// )
/// ```
class VNLGroupPositioned extends ParentDataWidget<VNLGroupParentData> {
  /// Creates a [VNLGroupPositioned].
  ///
  /// Parameters:
  ...
  /// Creates a [VNLGroupPositioned] that fills the entire group bounds.
  ///
  /// Sets all edges to 0, making the child fill the available space.
  const VNLGroupPositioned.fill({
    super.key,
    this.top = 0,
    this.left = 0,
    this.right = 0,
  ...
  /// Creates a [VNLGroupPositioned] from a [Rect].
  ///
  ...
  void applyParentData(RenderObject renderObject) {
    final parentData = renderObject.parentData as VNLGroupParentData;
  ...
  @override
  Type get debugTypicalAncestorWidgetClass => VNLGroupWidget;
}
