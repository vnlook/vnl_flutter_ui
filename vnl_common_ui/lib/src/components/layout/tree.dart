...
class TreeTheme extends ComponentThemeData {
  ...
  final VNLBranchLine? branchLine;
  ...
  ///
  /// Parameters:
  /// - [branchLine] (VNLBranchLine?): Visual style for lines connecting tree nodes
  ...
  ///
  /// Parameters:
  /// - [branchLine] (`ValueGetter<VNLBranchLine?>?`, optional): New branch line style.
...
class TreeNodeData<T> {
  ...
  final VNLBranchLine indentGuide;

  /// Whether this node is currently expanded.
  final bool expanded;

  /// List of depth information from root to this node.
  final List<VNLTreeNodeDepth> depth;
...
class VNLTreeNodeDepth {
  ...
  /// Creates a [VNLTreeNodeDepth] with the specified index and count.
  VNLTreeNodeDepth(this.childIndex, this.childCount);
}
...
class TreeView<T> extends StatefulWidget {
  ...
  ///
  /// Type: `VNLBranchLine?`. If null, uses the theme's branch line or VNLBranchLine.path.
  ...
  /// - [branchLine] (VNLBranchLine?, optional): Style for connecting lines
...
typedef _TreeWalker<T> = void Function(
    bool parentExpanded, TreeNode<T> node, List<VNLTreeNodeDepth> depth);
...
class _TreeViewState<T> extends State<TreeView<T>> {
  ...
  void _walkFlattened(
    ...
    bool parentExpanded,
    List<VNLTreeNodeDepth> depth,
  ) {
    for (int i = 0; i < nodes.length; i++) {
      final node = nodes[i];
      if (node is TreeItem<T>) {
        List<VNLTreeNodeDepth> newDepth = List.from(depth);
        newDepth.add(VNLTreeNodeDepth(i, nodes.length));
  ...
  Widget build(BuildContext context) {
    ...
    return Shortcuts(
      shortcuts: {
        if (allowMultiSelect) ...{
          ...
          LogicalKeySet(LogicalKeyboardKey.shift, LogicalKeyboardKey.arrowUp):
              const VNLDirectionalSelectTreeNodeIntent(false),
          LogicalKeySet(LogicalKeyboardKey.shift, LogicalKeyboardKey.arrowDown):
              const VNLDirectionalSelectTreeNodeIntent(true),
          LogicalKeySet(
      ...
      child: Actions(
        actions: {
          VNLSelectTreeNodeIntent: CallbackAction(
            onInvoke: (e) {
          ...
          VNLDirectionalSelectTreeNodeIntent: CallbackAction(
            onInvoke: (e) {
              final bool down = (e as VNLDirectionalSelectTreeNodeIntent).forward;
...
///
/// VNLBranchLine defines how visual connections are drawn between parent and child
...
/// - [VNLBranchLine.none] - No visual connections
/// - [VNLBranchLine.line] - Simple vertical lines
/// - [VNLBranchLine.path] - Connected path lines showing hierarchy
///
...
/// TreeView(
///   branchLine: VNLBranchLine.path, // Connected paths
///   // ... other properties
/// );
///
/// // Custom branch line implementation
/// class CustomBranchLine extends VNLBranchLine {
///   @override
///   Widget build(BuildContext context, List<VNLTreeNodeDepth> depth, int index) {
///     return CustomPaint(painter: MyCustomLinePainter());
///   }
/// }
/// ```
abstract class VNLBranchLine {
  /// Predefined branch line style with no visual connections.
  static const none = VNLIndentGuideNone();

  /// Predefined branch line style with simple vertical lines.
  static const line = VNLIndentGuideLine();

  /// Predefined branch line style with connected path lines.
  static const path = VNLIndentGuidePath();
  ...
  /// - [depth] (`List<VNLTreeNodeDepth>`): Hierarchical depth information
...
/// ```dart
/// TreeView(
///   branchLine: VNLBranchLine.none,
///   // ...
/// );
/// ```
class VNLIndentGuideNone implements VNLBranchLine {
  /// Creates an [VNLIndentGuideNone].
  const VNLIndentGuideNone();
...
/// ```dart
/// TreeView(
///   branchLine: VNLBranchLine.line,
///   // or with custom color:
///   branchLine: VNLIndentGuideLine(color: VNLColors.blue),
/// );
/// ```
class VNLIndentGuideLine implements VNLBranchLine {
  /// Custom color for the line. If null, uses the theme border color.
  final Color? color;

  /// Creates an [VNLIndentGuideLine] with optional custom color.
  const VNLIndentGuideLine({this.color});
...
///   branchLine: VNLBranchLine.path,
///   // or with custom color:
///   branchLine: VNLIndentGuidePath(color: VNLColors.grey),
/// );
/// ```
class VNLIndentGuidePath implements VNLBranchLine {
  /// Custom color for the path. If null, uses the theme border color.
  final Color? color;

  /// Creates an [VNLIndentGuidePath] with optional custom color.
  const VNLIndentGuidePath({this.color});
...
///
/// VNLTreeItemView provides a complete tree item interface that handles user
...
/// )
/// ```
class VNLTreeItemView extends StatefulWidget {
  ...
  /// Creates a [VNLTreeItemView] with comprehensive tree item functionality.
  ///
  ...
  @override
  State<VNLTreeItemView> createState() => _TreeItemViewState();
}

class _TreeItemViewState extends State<VNLTreeItemView> {
  ...
  @override
  void didUpdateWidget(covariant VNLTreeItemView oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final data = _data;
    assert(data != null, 'VNLTreeItemView must be a descendant of TreeView');
              ...
              shortcuts: {
                if (widget.onExpand != null &&
                  ...
                  LogicalKeySet(LogicalKeyboardKey.arrowRight):
                      const VNLExpandTreeNodeIntent(),
                if (widget.onExpand != null &&
                    (widget.expandable ?? data.node.children.isNotEmpty))
                  LogicalKeySet(LogicalKeyboardKey.arrowLeft):
                      const VNLCollapseTreeNodeIntent(),
              },
              actions: {
                ...
                VNLCollapseTreeNodeIntent: CallbackAction(
                  onInvoke: (e) {
                    if (widget.onExpand != null &&
                        (widget.expandable ?? data.node.children.isNotEmpty)) {
                ...
                VNLExpandTreeNodeIntent: CallbackAction(
                  onInvoke: (e) {
                    if (widget.onExpand != null &&
                        (widget.expandable ?? data.node.children.isNotEmpty)) {
...
class VNLExpandTreeNodeIntent extends Intent {
  /// Creates an [VNLExpandTreeNodeIntent].
  const VNLExpandTreeNodeIntent();
}
...
class VNLCollapseTreeNodeIntent extends Intent {
  /// Creates a [VNLCollapseTreeNodeIntent].
  const VNLCollapseTreeNodeIntent();
}
...
class VNLSelectTreeNodeIntent extends Intent {
  /// Creates a [VNLSelectTreeNodeIntent].
  const VNLSelectTreeNodeIntent();
}
...
class VNLDirectionalSelectTreeNodeIntent extends Intent {
  /// Whether to move forward (true) or backward (false) in the tree.
  final bool forward;

  /// Creates a [VNLDirectionalSelectTreeNodeIntent].
  const VNLDirectionalSelectTreeNodeIntent(this.forward);
}
