...
/// Theme configuration for [VNLTable] components.
///
/// [VNLTableTheme] provides comprehensive styling options for table components
...
/// );
/// ```
class VNLTableTheme extends ComponentThemeData {
  ...
  ///
  /// Type: `VNLTableCellTheme?`. Provides default styling for table cells
  /// including padding, borders, and text styles. Individual cells can
  /// override this theme.
  final VNLTableCellTheme? cellTheme;

  /// Creates a [VNLTableTheme].
  ///
  /// All parameters are optional and will fall back to theme defaults
  /// when not provided.
  ///
  /// Parameters:
  /// - [border] (Border?, optional): VNLTable container border
  /// - [backgroundColor] (Color?, optional): VNLTable background color
  /// - [borderRadius] (BorderRadiusGeometry?, optional): Corner radius
  /// - [cellTheme] (VNLTableCellTheme?, optional): Default cell styling
  ///
  /// Example:
  /// ```dart
  /// VNLTableTheme(
  ///   border: Border.all(color: VNLColors.grey),
  ...
  ///
  /// Returns a new [VNLTableTheme] instance with the same values as this theme,
  ...
  /// );
  /// ```
  VNLTableTheme copyWith({
    ValueGetter<Border?>? border,
    ValueGetter<Color?>? backgroundColor,
    ValueGetter<VNLTableCellTheme?>? cellTheme,
  }) {
    return VNLTableTheme(
...
/// to table dimensions. Used with [VNLResizableTable] to control
/// resize boundaries.
///
/// Example:
/// ```dart
/// VNLConstrainedTableSize(
///   min: 50.0,  // Minimum 50 pixels
///   max: 300.0, // Maximum 300 pixels
/// )
/// ```
class VNLConstrainedTableSize {
  ...
  /// Creates a [VNLConstrainedTableSize] with optional min and max values.
  const VNLConstrainedTableSize({
...
///
/// [VNLTableCellTheme] provides state-aware styling options for table cells
...
class VNLTableCellTheme {
  ...
  ///     Border.all(color: VNLColors.grey.shade300),
  ///   ),
  ///   backgroundColor: WidgetStateProperty.resolveWith((states) {
  ///     return states.contains(WidgetState.selected) ? VNLColors.blue.shade100 : null;
  ///   }),
  /// );
  /// ```
  const VNLTableCellTheme({
    this.border,
    this.backgroundColor,
    this.textStyle,
  });
  ...
  ///
  /// Returns a new [VNLTableCellTheme] instance with the same state properties
  ...
  /// ```dart
  /// final newTheme = originalTheme.copyWith(
  ///   backgroundColor: () => WidgetStateProperty.all(VNLColors.yellow.shade50),
...
/// Example:
/// ```dart
/// VNLResizableTableTheme(
///   tableTheme: VNLTableTheme(...),
///   resizerThickness: 2.0,
///   resizerColor: VNLColors.blue,
/// )
/// ```
class VNLResizableTableTheme extends ComponentThemeData {
  /// Base theme configuration for the table.
  final VNLTableTheme? tableTheme;
  ...
  /// Creates a [VNLResizableTableTheme].
  const VNLResizableTableTheme({
    this.tableTheme,
  ...
  bool operator ==(Object other) {
    ...
    return other is VNLResizableTableTheme &&
  ...
  ///
  /// Parameters:
  /// - [tableTheme] (`ValueGetter<VNLTableTheme?>?`, optional): New table theme.
  ...
  ///
  /// Returns: A new [VNLResizableTableTheme] with updated properties.
  VNLResizableTableTheme copyWith({
    ValueGetter<VNLTableTheme?>? tableTheme,
    ValueGetter<double?>? resizerThickness,
    ValueGetter<Color?>? resizerColor,
  }) {
    return VNLResizableTableTheme(
...
///   widthConstraints: {1: VNLConstrainedTableSize(min: 50, max: 300)},
/// );
/// ```
class VNLResizableTableController extends ChangeNotifier {
  Map<int, double>? _columnWidths;
  Map<int, double>? _rowHeights;
  final double _defaultColumnWidth;
  final double _defaultRowHeight;
  final VNLConstrainedTableSize? _defaultWidthConstraint;
  final VNLConstrainedTableSize? _defaultHeightConstraint;
  final Map<int, VNLConstrainedTableSize>? _widthConstraints;
  final Map<int, VNLConstrainedTableSize>? _heightConstraints;
  ...
  /// - [defaultWidthConstraint] (VNLConstrainedTableSize?): Default width constraints applied to all columns
  /// - [defaultHeightConstraint] (VNLConstrainedTableSize?): Default height constraints applied to all rows
  /// - [widthConstraints] (`Map<int, VNLConstrainedTableSize>?`): Per-column width constraints
  /// - [heightConstraints] (`Map<int, VNLConstrainedTableSize>?`): Per-row height constraints
  ///
  /// Example:
  /// ```dart
  /// VNLResizableTableController(
  ///   defaultColumnWidth: 100,
  ///   defaultRowHeight: 40,
  ///   widthConstraints: {
  ///     0: VNLConstrainedTableSize(min: 80, max: 200),
  ///   },
  /// )
  /// ```
  VNLResizableTableController({
    ...
    VNLConstrainedTableSize? defaultWidthConstraint,
    VNLConstrainedTableSize? defaultHeightConstraint,
    Map<int, VNLConstrainedTableSize>? widthConstraints,
    Map<int, VNLConstrainedTableSize>? heightConstraints,
...
/// ```dart
/// VNLResizableTable(
///   controller: VNLResizableTableController(),
///   rows: [
///     VNLTableRow(children: [Text('Cell 1'), Text('Cell 2')]),
///     VNLTableRow(children: [Text('Cell 3'), Text('Cell 4')]),
///   ],
/// )
/// ```
class VNLResizableTable extends StatefulWidget {
  ...
  final VNLResizableTableTheme? theme;
  ...
  /// Creates a [VNLResizableTable].
  ///
  /// Parameters:
  /// - [rows] (`List<VNLTableRow>`, required): VNLTable rows.
...
class _ResizableTableState extends State<VNLResizableTable> {
  ...
  VNLTableSize _width(int index) {
    return VNLFixedTableSize(widget.controller.getColumnWidth(index));
  }

  VNLTableSize _height(int index) {
    return VNLFixedTableSize(widget.controller.getRowHeight(index));
  }
...
class VNLTableCell {
  ...
  /// - VNLHover effects for column and row highlighting
...
/// Function that builds a [VNLTableCellTheme] based on context.
///
...
/// VNLTableRow(
///   cells: [
///     VNLTableCell(child: Text('Cell 1')),
///     VNLTableCell(child: Text('Cell 2')),
///   ],
///   selected: true,
///   cellTheme: VNLTableCellTheme(...),
/// )
...
class VNLTableRow {
  ...
  ///
  /// Returns [VNLTableCellTheme] with default or custom cell styling.
...
///
/// Extends [VNLTableRow] with default styling appropriate for footer rows,
...
/// VNLTableFooter(
///   cells: [
///     VNLTableCell(child: Text('Total: \$100')),
///     VNLTableCell(child: Text('Paid')),
///   ],
/// )
/// ```
class VNLTableFooter extends VNLTableRow {
  /// Creates a [VNLTableFooter].
  const VNLTableFooter({required super.cells, super.cellTheme});
...
///
/// Extends [VNLTableRow] with default styling appropriate for header rows,
...
/// VNLTableHeader(
///   cells: [
///     VNLTableCell(child: Text('Name')),
///     VNLTableCell(child: Text('Age')),
///     VNLTableCell(child: Text('City')),
///   ],
/// )
/// ```
class VNLTableHeader extends VNLTableRow {
  /// Creates a [VNLTableHeader].
  const VNLTableHeader({required super.cells, super.cellTheme});
...
///
/// [VNLTable] provides a comprehensive table component with advanced layout capabilities
...
/// - **Interactive States**: VNLHover effects and selection states with visual feedback
...
/// - Variable column widths via [VNLTableSize] strategies
/// - Dynamic row heights based on content
/// - Complex cell spanning calculations
/// - Efficient rendering with viewport culling
///
/// ## Sizing Strategies
/// - [VNLFlexTableSize]: Proportional sizing like CSS flex
/// - [VNLFixedTableSize]: Absolute pixel sizes
/// - [VNLIntrinsicTableSize]: Size based on content
///
/// Example:
/// ```dart
/// VNLTable(
...
///     ]),
///     VNLTableRow(cells: [
///       VNLTableCell(child: Text('John Doe')),
///       VNLTableCell(child: Text('25')),
///       VNLTableCell(child: Text('New York')),
///     ]),
///   ],
///   columnWidths: {
///     0: VNLFlexTableSize(flex: 2),
///     1: VNLFixedTableSize(width: 80),
///     2: VNLFlexTableSize(flex: 1),
///   },
/// );
/// ```
class VNLTable extends StatefulWidget {
  ...
  ///
  /// Type: `List<VNLTableRow>`. Contains the table data organized as rows.
  /// Can include [VNLTableRow], [VNLTableHeader], and [VNLTableFooter] instances.
  /// Each row contains a list of [VNLTableCell] widgets.
  final List<VNLTableRow> rows;

  /// Default sizing strategy for all columns.
  ///
  /// Type: `VNLTableSize`. Used when no specific width is provided in
  /// [columnWidths]. Defaults to [VNLFlexTableSize] for proportional sizing.
  final VNLTableSize defaultColumnWidth;

  /// Default sizing strategy for all rows.
  ///
  /// Type: `VNLTableSize`. Used when no specific height is provided in
  /// [rowHeights]. Defaults to [VNLIntrinsicTableSize] for content-based sizing.
  final VNLTableSize defaultRowHeight;

  /// Specific column width overrides.
  ///
  /// Type: `Map<int, VNLTableSize>?`. Maps column indices to specific sizing
  /// strategies. Overrides [defaultColumnWidth] for specified columns.
  final Map<int, VNLTableSize>? columnWidths;

  /// Specific row height overrides.
  ///
  /// Type: `Map<int, VNLTableSize>?`. Maps row indices to specific sizing
  /// strategies. Overrides [defaultRowHeight] for specified rows.
  final Map<int, VNLTableSize>? rowHeights;
  ...
  ///
  /// Type: `VNLFrozenTableData?`. Specifies which cells remain visible
  ...
  /// Creates a [VNLTable] widget.
  ///
  /// The table displays data organized in rows and cells with flexible
  /// sizing and interactive features.
  ///
  /// Parameters:
  /// - [rows] (`List<VNLTableRow>`, required): VNLTable data organized as rows
  /// - [defaultColumnWidth] (VNLTableSize, default: VNLFlexTableSize()): Default column sizing
  /// - [defaultRowHeight] (VNLTableSize, default: VNLIntrinsicTableSize()): Default row sizing
  /// - [columnWidths] (`Map<int, VNLTableSize>?`, optional): Column-specific sizes
  /// - [rowHeights] (`Map<int, VNLTableSize>?`, optional): Row-specific sizes
  /// - [clipBehavior] (Clip, default: Clip.hardEdge): Content clipping behavior
  /// - [theme] (VNLTableTheme?, optional): Visual styling configuration
  /// - [frozenCells] (VNLFrozenTableData?, optional): Frozen cell configuration
  ...
  /// VNLTable(
  ///   rows: [
  ///     VNLTableHeader(cells: [VNLTableCell(child: Text('Header'))]),
  ///     VNLTableRow(cells: [VNLTableCell(child: Text('Data'))]),
  ///   ],
  ///   columnWidths: {0: VNLFixedTableSize(width: 200)},
  /// );
  /// ```
  const VNLTable({
    super.key,
    required this.rows,
    this.defaultColumnWidth = const VNLFlexTableSize(),
    this.defaultRowHeight = const VNLIntrinsicTableSize(),
    this.columnWidths,
    this.rowHeights,
    this.clipBehavior = Clip.hardEdge,
    this.theme,
...
/// Example:
/// ```dart
/// VNLTableRef(0, 2) // References rows/columns 0 and 1
/// VNLTableRef(5)    // References row/column 5 with span of 1
/// ```
class VNLTableRef {
  ...
  /// Creates a [VNLTableRef].
  const VNLTableRef(this.index, [this.span = 1]);
...
/// ```dart
/// VNLFrozenTableData(
///   frozenRows: [VNLTableRef(0)],      // Freeze first row (header)
///   frozenColumns: [VNLTableRef(0, 2)], // Freeze first two columns
/// )
/// ```
class VNLFrozenTableData {
  /// Rows that should be frozen during vertical scrolling.
  final Iterable<VNLTableRef> frozenRows;

  /// Columns that should be frozen during horizontal scrolling.
  final Iterable<VNLTableRef> frozenColumns;

  /// Creates a [VNLFrozenTableData].
  const VNLFrozenTableData(
...
class VNLTableParentData extends ContainerBoxParentData<RenderBox> {
...
/// )
/// ```
class VNLRawCell extends ParentDataWidget<VNLTableParentData> {
  ...
  void applyParentData(RenderObject renderObject) {
    final parentData = renderObject.parentData as VNLTableParentData;
    ...
    if (needsLayout) {
      final table = renderObject.parent as VNLRenderTableLayout;
      table.markNeedsLayout();
    }
  }

  @override
  Type get debugTypicalAncestorWidgetClass => VNLRawTableLayout;
}
...
abstract base class VNLTableSize {
  /// Creates a [VNLTableSize].
  const VNLTableSize();
}

/// VNLTable size mode that distributes available space using flex factors.
///
...
/// Example:
/// ```dart
/// VNLFlexTableSize(flex: 2.0, fit: FlexFit.tight)
/// ```
final class VNLFlexTableSize extends VNLTableSize {
  ...
  /// Creates a [VNLFlexTableSize].
  const VNLFlexTableSize({this.flex = 1, this.fit = FlexFit.tight});
}

/// VNLTable size mode with a fixed pixel value.
///
...
/// Example:
/// ```dart
/// VNLFixedTableSize(150.0) // 150 pixels
/// ```
final class VNLFixedTableSize extends VNLTableSize {
  /// The fixed size value in pixels.
  final double value;

  /// Creates a [VNLFixedTableSize] with the specified pixel value.
  const VNLFixedTableSize(this.value);
}

/// VNLTable size mode that uses the intrinsic size of cell content.
///
...
/// VNLIntrinsicTableSize() // Size based on content
/// ```
final class VNLIntrinsicTableSize extends VNLTableSize {
  /// Creates an [VNLIntrinsicTableSize].
  const VNLIntrinsicTableSize();
}

/// VNLTable size mode that uses a fractional value of the table's size.
///
...
/// ```
final class FractionalTableSize extends VNLTableSize {
...
/// ```dart
/// VNLRawTableLayout(
///   width: (index) => VNLFlexTableSize(),
///   height: (index) => VNLFixedTableSize(50),
///   clipBehavior: Clip.hardEdge,
///   children: [...],
/// )
/// ```
class VNLRawTableLayout extends MultiChildRenderObjectWidget {
  /// Creates a [VNLRawTableLayout].
  ///
  /// Parameters:
  /// - [children] (`List<Widget>`, optional): VNLTable cell widgets.
  ...
  @override
  VNLRenderTableLayout createRenderObject(BuildContext context) {
    return VNLRenderTableLayout(
        width: width,
        height: height,
  ...
  void updateRenderObject(
      BuildContext context, VNLRenderTableLayout renderObject) {
...
/// Function that provides a [VNLTableSize] for a given index.
///
...
///   if (index == 0) return VNLFixedTableSize(50);
///   return VNLFlexTableSize(flex: 1);
/// };
/// ```
typedef TableSizeSupplier = VNLTableSize Function(int index);
...
///
/// See also:
/// - [VNLTableSize], which defines sizing strategies for columns and rows
/// - [VNLTableLayoutResult], which contains the computed layout dimensions
class VNLRenderTableLayout extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, VNLTableParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, VNLTableParentData> {
  ...
  VNLTableLayoutResult? _layoutResult;
  ...
  void setupParentData(RenderObject child) {
    if (child.parentData is! VNLTableParentData) {
      child.parentData = VNLTableParentData();
    }
  }
  ...
  bool hitTestChildren(BoxHitTestResult result, {required Offset position}) {
    ...
    while (child != null) {
      final parentData = child.parentData as VNLTableParentData;
  ...
  ///
  /// The layout algorithm respects size constraints from [VNLTableSize] objects
  ...
  ///
  /// Returns [VNLTableLayoutResult] containing computed dimensions and layout metadata.
  VNLTableLayoutResult computeTableSize(BoxConstraints constraints,
    ...
    for (int r = 0; r <= maxRow; r++) {
      ...
      if (heightConstraint is VNLFlexTableSize &&
      ...
      } else if (heightConstraint is VNLFixedTableSize) {
    ...
    for (int c = 0; c <= maxColumn; c++) {
      ...
      if (widthConstraint is VNLFlexTableSize && constraints.hasBoundedWidth) {
        flexWidth += widthConstraint.flex;
        hasFlexWidth = true;
        if (widthConstraint.fit == FlexFit.tight) {
          hasTightFlexWidth = true;
        }
      } else if (widthConstraint is VNLFixedTableSize) {
    ...
    while (child != null) {
      ...
      if (parentData.computeSize) {
        ...
        if (column != null && row != null) {
          ...
          if (widthConstraint is VNLIntrinsicTableSize ||
              (widthConstraint is VNLFlexTableSize && intrinsicComputer != null)) {
          ...
          if (heightConstraint is VNLIntrinsicTableSize ||
              (heightConstraint is VNLFlexTableSize &&
                  intrinsicComputer != null)) {
    ...
    if (intrinsicComputer == null) {
      ...
      if (hasFlexWidth) {
        for (int c = 0; c <= maxColumn; c++) {
          final widthConstraint = _width(c);
          if (widthConstraint is VNLFlexTableSize) {
  ...
  // delegate from VNLTableLayoutResult, with read-only view
...
class VNLTableLayoutResult {
  ...
  /// Creates a [VNLTableLayoutResult].
  VNLTableLayoutResult({
