...
/// Theme data for customizing [VNLPagination] widget appearance.
///
/// This class defines the visual and behavioral properties that can be applied to
/// [VNLPagination] widgets, including spacing between controls and label display
/// preferences. These properties can be set at the theme level to provide
/// consistent styling across the application.
class VNLPaginationTheme extends ComponentThemeData {
  ...
  const VNLPaginationTheme({
    this.gap,
    this.showLabel,
  });

  /// Returns a copy of this theme with the given fields replaced.
  VNLPaginationTheme copyWith({
    ValueGetter<double?>? gap,
    ValueGetter<bool?>? showLabel,
  }) {
    return VNLPaginationTheme(
  ...
  bool operator ==(Object other) {
    ...
    return other is VNLPaginationTheme &&
        other.gap == gap &&
...
///
/// [VNLPagination] provides an intuitive interface for navigating through paginated
...
class VNLPagination extends StatelessWidget {
  ...
  Widget _buildPreviousLabel(
      VNLookLocalizations localizations, bool showLabel) {
    if (showLabel) {
  ...
  Widget _buildNextLabel(VNLookLocalizations localizations, bool showLabel) {
    if (showLabel) {
      return VNLGhostButton(
