...
class _FormExampleState extends State<FormExample> {
  ...
  Widget build(BuildContext context) {
    return DocsPage(
      ...
      onThisPage: {
        ...
        'Example: VNLTable Layout': example1Key,
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'The VNLForm widget provides a structured way to collect, validate, and '
          ...
          Column(
            ...
            children: [
              const Text(
                'VNLForm — the container widget that manages state and handles submission.',
              ).li(),
              const Text(
          ...
          // ── VNLForm Keys ──
          const Text('VNLForm Keys').h2().anchored(formKeysKey),
          const Text(
          ...
          const VNLAlert(
            ...
            content: Text(
              'A VNLTextField reports String values. If you use FormKey<int>(\'name\') '
          ...
          const Text(
            ...
            'all pass, calls the VNLForm\'s onSubmit callback with a typed value map.',
          ).p(),
          const Text(
            ...
            'VNLSubmitButton widget which automatically handles loading and error '
            'states. VNLSubmitButton disables itself while async validators are '
          ...
          const Text(
            'Most form inputs have a Controlled variant (e.g. VNLControlledCheckbox, '
            'VNLControlledDatePicker, VNLControlledSwitch) that manages its own state '
            ...
            'VNLFormController already tracks the value — you just need the widget '
          ...
          const Text(
            'Any form-capable widget (VNLTextField, VNLCheckbox, etc.) placed inside '
            'a VNLForm will automatically register with the VNLFormController, even '
          ...
          // ── Full Examples ──
          const Text('Example: VNLTable Layout').h2().anchored(example1Key),
          const Text(
            'A basic registration form using VNLFormTableLayout with cross-field '
  ...
  VNLTableCell _padded(Widget child) {
    return VNLTableCell(
      child: Container(
  ...
  VNLTableCell _headerCell(String text) {
    return VNLTableCell(
      child: Container(
        padding: const EdgeInsets.all(8),
  ...
  static const _keyEntries = <(String, String, String)>[
    ('TextFieldKey', 'String', 'VNLTextField'),
    ...
    ('StarRatingKey', 'double', 'VNLStarRating'),
    ('SelectKey<T>', 'T', 'Select'),
    ('MultiSelectKey<T>', 'Iterable<T>', 'MultiSelect'),
    ('RadioGroupKey', 'int', 'RadioGroup'),
    ('RadioCardKey', 'int', 'RadioCard'),
    ('DatePickerKey', 'DateTime', 'VNLDatePicker'),
    ('DateInputKey', 'DateTime', 'VNLDateInput'),
    ('TimePickerKey', 'TimeOfDay', 'VNLTimePicker'),
  ...
  static const _controlledEntries = <(String, String, String)>[
    ...
    ('VNLTimePicker', 'VNLControlledTimePicker', 'TimeOfDay?'),
    ('VNLSlider', 'VNLControlledSlider', 'VNLSliderValue'),
    ('VNLStarRating', 'VNLControlledStarRating', 'double'),
    ('Select<T>', 'ControlledSelect<T>', 'T?'),
    ('MultiSelect<T>', 'ControlledMultiSelect<T>', 'List<T>'),
    ('RadioGroup<T>', 'ControlledRadioGroup<T>', 'T?'),
    ('VNLColorInput', 'VNLControlledColorInput', 'Color?'),
  ];
  ...
  Widget _buildPitfalls() {
    return VNLAccordion(
      items: [
        ...
        const VNLAccordionItem(
          trigger: VNLAccordionTrigger(
            child: Text('VNLForm-capable widgets inside VNLForm without FormField'),
          ),
          content: Text(
            'Any form-capable widget (VNLTextField, VNLCheckbox, etc.) placed inside '
            'a VNLForm will automatically try to register with the VNLFormController, '
            'even without a FormField wrapper. If you have a search VNLTextField '
            'or a filter VNLCheckbox that should NOT participate in form validation '
        ...
        VNLAccordionItem(
          ...
          content: const Text(
                  'Some validators (like VNLEmailValidator, VNLRegexValidator, VNLURLValidator, etc.) '
                  'will return valid if the string is empty. This means a blank field will pass '
                  'validation unless you also add VNLNotEmptyValidator. Always combine VNLNotEmptyValidator '
  ...
  static const _validatorEntries = <(String, String)>[
    ...
    ('VNLRegexValidator', 'Checks if a string matches a regular expression.'),
    ('VNLEmailValidator', 'Checks if a string is a valid email address.'),
    ('VNLURLValidator', 'Checks if a string is a valid URL.'),
    (
